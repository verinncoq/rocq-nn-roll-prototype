...
Lemma sum_n_m_shift {G: AbelianMonoid}:
    forall (f: nat -> AbelianMonoid.sort G) n m shift,
...
Definition coeff_colvec {m: nat} (x0: RS) (V: colvec m) (i:nat) :=
  coeff_mat x0 V i 0.

(* Construction of a colvec *)
Definition mk_colvec m (f: nat -> RS) : colvec m := 
...
Theorem dot_scalar_mult:
    ...
    dot (scalar_mult c v1) v2 = c * (dot v1 v2).
Proof.
    intros dim c v1 v2.
    unfold dot.
    unfold Mmult.
...
Theorem dot_null_vector:
    forall dim x, dot (null_vector dim) x = RSzero.
Proof.
  intros dim x.
  unfold dot.
  unfold Mmult.
...
Theorem dot_comm:
    forall dim (v1: colvec dim) (v2: colvec dim),
        dot v1 v2 = dot v2 v1.
Proof.
    intros dim v1 v2.
    unfold dot.
...
Proof. 
    ...
    rewrite <- (mk_matrix_bij RSzero v) at 1.
    apply mk_matrix_ext.
    intros i j Hi Hj.
    unfold coeff_colvec.
...
Proof.  
    intros d1 d2 v1 v2.
    rewrite <- (mk_matrix_bij RSzero v1).
    rewrite <- (mk_matrix_bij RSzero v2).
    split.
    {
      intros Hequal.
    ...
    }
    {
      ...
      specialize (Hext (fun i _ : nat => 
                          if i <? d1 then 
                            coeff_colvec RSzero
                              (mk_matrix d1 1 (coeff_mat RSzero v1)) i
                          else 
                            RSzero)).
      specialize (Hext (fun i _ : nat => 
                          if i <? d1 then 
                            coeff_colvec RSzero 
                              (mk_matrix d1 1 (coeff_mat RSzero v2)) i
                          else
                            RSzero)).
...
Definition extend_colvec_on_top {n: nat} (v: colvec n) (new_dim: nat) :=
    mk_colvec (RSOPM:=RSOPM) new_dim (
        ...
        | true => RSzero
        | false => coeff_colvec RSzero v (i - (new_dim - n))
        end
    ).
    ...
    }
    {
      ...
      specialize (Hext (fun i _ : nat => 
                          if i <? d1 then 
                            ...
                            coeff_colvec RSzero 
                              (mk_matrix d2 1 (coeff_mat RSzero v1)) (i - d1))).
      specialize (Hext (fun i _ : nat => 
                          if i <? d1 then 
                            RSzero
                          else 
                            coeff_colvec RSzero 
                              (mk_matrix d2 1 (coeff_mat RSzero v2)) (i - d1))).
      destruct Hext as [Hext1 Hext2].               
      specialize (Hext2 Hequal).
...
Proof.
    ...
    * destruct d2.
      ...
      - unfold extend_colvec_on_top.
        ...
        repeat (rewrite coeff_mat_bij; try lia).
        unfold sum_n. simpl.
        rewrite <- (plus_zero_l (sum_n_m _ _ (d2))).
        rewrite (sum_n_m_Chasles _ _ d1 (d1 + S d2)); try lia.
        apply f_equal2_plus_RS.
        * simpl.
...
Proof.
    ...
    - apply not_lt in Helt.
      ...
      * unfold extend_colvec_on_top. unfold block_diag_matrix.
        ...
        * rewrite <- (plus_zero_l (sum_n _ (pred n2))).
          unfold sum_n. 
...
Lemma colvec_split:
    forall dim1 dim2 v,
        exists v1 v2,
            v1 = (mk_colvec dim1 (fun i => coeff_colvec RSzero v i)) /\
            v2 = (mk_colvec dim2 (fun i => coeff_colvec RSzero v (i + dim1))) /\
            v = colvec_concat v1 v2.
Proof.
    intros dim1 dim2 v.
    exists (mk_colvec dim1 (fun i => coeff_colvec RSzero v i)).
    exists (mk_colvec dim2 (fun i => coeff_colvec RSzero v (i + dim1))).
    ...
    - induction dim2.
      * apply unique_colvec_0.
    all: (
        rewrite <- (mk_matrix_bij RSzero v);
        apply mk_matrix_ext;
        intros i j Hi Hj
    ).
      * rewrite coeff_mat_bij; try lia.
